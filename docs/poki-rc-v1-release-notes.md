...
- RC source commit: the full `sourceCommit` in `releases/poki-rc-v1-manifest.json`, generated from committed build inputs. The ZIP embeds `poki-rc-v1+<12-character source SHA>` through the existing `VITE_BUILD_VERSION` path. A later documentation/evidence-only commit can be the branch tip without changing the ZIP's source identity.
